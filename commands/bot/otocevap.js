...
message.channel.send(basarili)
  ...
  for (var i = 0; i < (db.fetch(`${message.guild.id}e?otocevap.yazılar`) ? db.fetch(`${message.guild.id}.otocevap.yazılar`).length : 0); i++) {
    if (args[1] == yazılar[i]) {
        const basarili = new Discord.MessageEmbed()
       .setAuthor(`${message.author.username},`, message.author.avatarURL({ dynamic: true }))
       ...
       .setFooter(footer)
      const asd = db.fetch(`${message.guild.id}e?otocevap.yazılar`)
      asd.splice(i,1)
      db.set(`${message.guild.id}e?otocevap.yazılar`, asd);
      db.push(`${message.guild.id}e?otocevap.yazılar`, bilmpye[0])
      const asdd = db.fetch(`${message.guild.id}e?otocevap.cevaplar`)
      asdd.splice(i,1)
      db.set(`${message.guild.id}e?otocevap.cevaplar`, asdd);
      db.push(`${message.guild.id}e?otocevap.cevaplar`, bilmpye[1])
      message.channel.send(basarili)
  }}
} else {
