...
if(message.attachments.first()) {
  try {
    ...
    return message.guild.emojis.create(message.attachments.first().url, message.attachments.first().name.split('.')[0], { reason: `Ewing Hızlı Emoji `+message.author.tag }).then(emoji => message.channel.send(`Emoji yüklendi: ${message.guild.emojis.cache.get(emoji.id)}`))
  ...
  if(args.slice(0).join(' ').startsWith('https://') && args.slice(0).join(' ').includes('discord')) {
    try {
      ...
      return message.guild.emojis.create(args.slice(0).join(' '), args.slice(0).join(' ').split('/')[4].split('.')[0], { reason: `ewing emoji yükle: `+message.author.tag }).then(emoji => message.channel.send(`Emoji yüklendi: ${message.guild.emojis.cache.get(emoji.id)}`)).catch(err => message.channel.send(`Bir hata oluştu. Lütfen; 
- Sunucuda emoji yüklemek için yer olduğuna, 
- Koyduğunuz dosyanın bir fotoğraf/gif olduğuna, 
- 256kb boyutundan küçük olduğuna,
emin olun ve tekrar deneyin.`))
